angular.module('login').service('LoginService', ["$rootScope", "$location", "$http", function ($rootScope, $location, $http) {
    var vm = this;
    vm.check = check;
    vm.getNick = getNick;
...
angular.module('login').controller('LoginController', ["$window", "$rootScope", "$location", "$http", "LoginService", "$scope", "LoginSocketService", "UserService", "LocalizationService", "LoadService", "ChatController", function ($window, $rootScope, $location, $http, LoginService, $scope, LoginSocketService, UserService, LocalizationService, LoadService, ChatController) {
    var vm = this;
    vm.username = "";
    vm.pass = "";
    vm.submit = submit;
...
angular.module('login').run(["$location", "UserService", function ($location, UserService) {
    var vm = this;
    vm.currentUser = UserService.getCurrentUser();
...
angular.module('chat').controller('ChatController', ["$window", "$rootScope", "$scope", "$compile", "LoginService", "$location", "ChatSocketService", "UserService", "LocalizationService", "LoadService", "$timeout", function ($window, $rootScope, $scope, $compile, LoginService, $location, ChatSocketService, UserService, LocalizationService, LoadService, $timeout) {
    var vm = this;
    var self = this;
...
angular.module('chat').directive('scrollDirective', ["$rootScope", "$timeout", function ($rootScope, $timeout) {
...
angular.module("myApp").controller('MenuController', ["$element", "$by", "LocalizationService", "$rootScope", function ($element, $by, LocalizationService, $rootScope) {
    var vm = this;
    vm.menuClicked = true;
    vm.menuClickedFunc = menuClickedFunc;
    vm.toggleMenu = toggleMenu;
    var menuButton = $element($by.css('#menuShowButton'));
    menuButton.on('click', vm.menuClickedFunc());
    if (vm.menuClicked === false) {
        console.log("in");
        menuButton.attr('right', 0);
    }
    else {
        console.log("in 100");
        menuButton.attr('right', -100);
    }
